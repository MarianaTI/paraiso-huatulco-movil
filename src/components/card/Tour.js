...
const Tour = ({ name, code, hour, availability, count, max, adults, kids, babies }) => {
  return (
    <div className="mt-3 tour-card">
      <div className="d-flex flex-column">
        ...
        <span style={{fontSize: 12}}>Horario: {hour}</span>
      </div>
      ...
      <div className="d-flex justify-content-between">
        <span style={{fontSize: 12, fontWeight: 600}}>Lugares disponibles: </span>
        <span>{count ? count : "Ilimitado"}</span>
      </div>
      <div className="d-flex justify-content-between">
        <span style={{fontSize: 12, fontWeight: 600}}>Máximo personas: </span>
        <span>{max ? max : "Ilimitado"}</span>
      </div>
